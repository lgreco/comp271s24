public class TrainLine {
    ...
    /**
     * Produces a list of stations in the order of traversal. Use StringBuilder to
     * mutable objects
     */
    ...
    /**
     * Returns a string with the names of the train stations along the trainline
     * object, in reverse order. If the line is empty (head==null) the method returs
     * an empty line message. For illustrative purposes, the method is implemented
     * with strings. A more practical implementation with StringBuilder may obscure
     * the simple technique for inverse listing.
     * 
     * The method works in the same way as if we were writing the names of the
     * stations on a sheet of paper beginning from its bottom and working our way
     * up. The first station would appear at the bottom, the second station above
     * that, etc, until the last station appears at the top. The same can be done
     * when building a string. Usually we add new content to a string after its end.
     * Here we add it at the beginning.
     * 
     * @return String with station names in reverse order
     */
    public String reverseListStations() {
        String reverseList = new String(EMPTY_LINE_MESSAGE);
        if (this.head != null) {
            reverseList = this.head.getName();
            Station current = this.head.getNext();
            while (current != null) {
                // At the current station name at the beginning of the string
                reverseList = current.getName() + NEWLINE + reverseList;
                // Using a StringBuilder and String.format may obscure the reverse stacking of
                // names:
                // sb.append(String.format("%s\n%s", current.getName(), sb.toString()))
                // That's why the method is implemented with Strings, for illustration
                current = current.getNext();
            }
        }
        return reverseList;
